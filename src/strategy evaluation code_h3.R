...
data$strategy_return_logit <- ifelse(
  data$predicted_prob_logit_h3 > 0.5,  # Threshold of 0.5 for investment in stocks
...
data$strategy_return_lasso <- ifelse(
  data$predicted_prob_lasso_h3 > 0.5,  # Threshold of 0.5 for investment in stocks
  data$original_ret,          # Invest in stocks: Use the stock return
  data$original_tbl         # Invest in risk-free asset: Use T-bill rate
)
data$strategy_return_boosting_gbm <- ifelse(
  data$predicted_prob_boosting_h3 > 0.5,  # Threshold of 0.5 for investment in stocks
  data$original_ret,          # Invest in stocks: Use the stock return
  data$original_tbl         # Invest in risk-free asset: Use T-bill rate
)
data$strategy_return_boosting_gbm_sample_mean <- ifelse(
  data$predicted_prob_boosting_sample_mean_h3 > 0.5,  # Threshold of 0.5 for investment in stocks
  data$original_ret,          # Invest in stocks: Use the stock return
  data$original_tbl         # Invest in risk-free asset: Use T-bill rate
)
...
data$strategy_return_random_forest <- ifelse(
  data$predicted_prob_random_forest_h3 > 0.5,  # Threshold of 0.5 for investment in stocks
  data$original_ret,          # Invest in stocks: Use the stock return
  data$original_tbl         # Invest in risk-free asset: Use T-bill rate
)
...
p1 <- ggplot(data, aes(x = date)) +
  ...
  labs(title = "Cumulative Portfolio Returns Using the 50% Rule For H3", y = "Cumulative Return", x = "Date", color = "Model") +
  theme_minimal() +
...
sample_avg_threshold <- 1-mean(data_train$market_state)  # Calculate sample average of bear markets

data$strategy_return_avg_logit <- ifelse(
  data$predicted_prob_logit_h3 > sample_avg_threshold,
  data$original_ret,          # Invest in stocks: Use the stock return
  data$original_tbl
)
data$strategy_return_avg_lasso <- ifelse(
  data$predicted_prob_lasso_h3 > sample_avg_threshold,
  data$original_ret,          # Invest in stocks: Use the stock return
  data$original_tbl
)
data$strategy_return_avg_boosting_gbm <- ifelse(
  data$predicted_prob_boosting_h3 > sample_avg_threshold,
  data$original_ret,          # Invest in stocks: Use the stock return
  data$original_tbl
)
data$strategy_return_avg_boosting_sample_mean <- ifelse(
  data$predicted_prob_boosting_sample_mean_h3 > sample_avg_threshold,
  data$original_ret,          # Invest in stocks: Use the stock return
  data$original_tbl
)
data$strategy_return_avg_boosting_xgb <- ifelse(
  data$predicted_prob_boosting_xgb_h3 > sample_avg_threshold,
  data$original_ret,          # Invest in stocks: Use the stock return
  data$original_tbl
)
data$strategy_return_avg_bagging <- ifelse(
  data$predicted_prob_bagging_h3 > sample_avg_threshold,
  data$original_ret,          # Invest in stocks: Use the stock return
  data$original_tbl
)
data$strategy_return_avg_random_forest <- ifelse(
  data$predicted_prob_random_forest_h3 > sample_avg_threshold,
  data$original_ret,          # Invest in stocks: Use the stock return
  data$original_tbl
)
...
p2 <- ggplot(data, aes(x = date)) +
  ...
  labs(title = "Cumulative Portfolio Returns Using Sample Average Threshold For H3", y = "Cumulative Return", x = "Date", color = "Model") +
  theme_minimal() +
  theme(legend.position = "bottom", axis.title.x = element_blank())
