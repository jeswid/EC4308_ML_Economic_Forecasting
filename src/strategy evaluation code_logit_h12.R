...
# Assuming 'data' contains 'predicted_prob_logit_h12' (forecasted probability of bear market)
# and 'market_state' (actual market state: 1 = Bear, 0 = Bull)

results_logit_h12 <- data %>%
  filter(!is.na(predicted_prob_logit_h12))

# Define portfolio strategy based on forecast probabilities
data$strategy_return <- ifelse(
  data$predicted_prob_logit_h12 < 0.5,  # Threshold of 0.5 for investment in stocks
...
data$strategy_return_avg <- ifelse(
  data$predicted_prob_logit_h12 < sample_avg_threshold,
  data$ret,
  data$tbl
)
...
# 2. Define the strategy: Buy stocks if predicted_prob_logit_h12 < ma_threshold
data <- data %>%
  filter(!is.na(ma_threshold))
data$strategy_return_2sided_avg <- ifelse(
  data$predicted_prob_logit_h12 < data$ma_threshold, 
  data$ret,  # Buy stocks
