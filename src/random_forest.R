...
rf.rolling.window=function(X,y,ntest, h){
  ...
  for(i in ntest:1){
    Y.window=y[(1+ntest-i):(length(y)-i)] #define the estimation window
...
bagging.rolling.window=function(X,y,ntest, h){
  ...
  for(i in ntest:1){
    Y.window=y[(1+ntest-i):(length(y)-i)] #define the estimation window 
    X.window=X[(1+ntest-i):(nrow(X)-i),]
    rffit=runbagging(X.window, Y.window, h)#call the function to fit the bagging model and generate h-step forecast
    save.pred[(1+ntest-i),]=as.numeric(levels(rffit$pred)[rffit$pred])  #save the forecast
