...
# Assuming 'data' contains 'predicted_prob_logit_h6' (forecasted probability of bear market)
# and 'market_state' (actual market state: 1 = Bear, 0 = Bull)

results_logit_h6 <- data %>%
...
data$strategy_return_2sided_avg <- ifelse(
  data$predicted_prob_logit_h6 < data$ma_threshold, 
  data$ret,  # Buy stocks
