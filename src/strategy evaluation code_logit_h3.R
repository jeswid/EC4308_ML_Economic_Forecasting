...
# Assuming 'data' contains 'predicted_prob_logit_h3' (forecasted probability of bear market)
# and 'market_state' (actual market state: 1 = Bear, 0 = Bull)

results_logit_h3 <- data %>%
...
# 2. Define the strategy: Buy stocks if predicted_prob_logit_h3 < ma_threshold
data <- data %>%
  filter(!is.na(ma_threshold))
data$strategy_return_2sided_avg <- ifelse(
  data$predicted_prob_logit_h3 < data$ma_threshold, 
  data$ret,  # Buy stocks
